...
import { useUploadNewProjectVersion } from '../../api/use-upload-new-project-version';
import { useProjectVersionModalController } from '../../hooks/use-version-modal-controller';

const versionSchema = createProjectDocumentSchema.partial();
export const UploadProjectVersionModal = () => {
  const { versionId, close } = useProjectVersionModalController();

  const { data, isPending, isError } = useCurrentUser();

  const workspaceId = useWorkspaceId();
  const projectId = useProjectId();
  const { mutateAsync } = useUploadNewProjectVersion();
  ...
  }, [data?.profile?.name, setValue, workspaceId, projectId]);
