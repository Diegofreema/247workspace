...
import { useCreateWorkspaceDocumentModalController } from '../../hooks/use-create-workspace-document';

export const CreateWorkspaceDocumentModal = () => {
  const { folderId, close } = useCreateWorkspaceDocumentModalController();

  const { data, isPending, isError } = useCurrentUser();

  const workspaceId = useWorkspaceId();

  const { mutateAsync } = useUploadWorkspaceDocument();
  ...
  const {
    ...
    setValue,
  } = useForm<z.infer<typeof createWorkspaceDocumentSchema>>({
    defaultValues: {
      name: '',
      documentUrl: '',
    ...
    resolver: zodResolver(createWorkspaceDocumentSchema),
  });

  useEffect(() => {
    if (data?.profile?.name && workspaceId && folderId) {
      setValue('uploadedBy', data?.profile?.name as string);
      setValue('workspaceId', workspaceId);
      setValue('folderId', folderId);
    }
  }, [data?.profile?.name, setValue, workspaceId, folderId]);

  const onSubmit = async (
    data: z.infer<typeof createWorkspaceDocumentSchema>
  ) => {
    const finalValues = {
      ...data,
