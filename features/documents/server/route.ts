...
const app = new Hono()
  ...
  .get(
    '/project-version-history/:versionId',
    sessionMiddleware,
    zValidator('query', z.object({ page: z.string() })),
    async (c) => {
      const databases = c.get('databases');
  ...
  .post(
    '/upload-new-project-version/:versionId',
    sessionMiddleware,
    zValidator('form', createProjectDocumentSchema.partial()),
    async (c) => {
      const databases = c.get('databases');
      const storage = c.get('storage');
      const user = c.get('user');
      ...
      const previousDocuments =
        await databases.listDocuments<ProjectDocumentType>(
          DATABASE_ID,
          DOCUMENT_ID,
          [
            Query.equal('versionId', versionId),
